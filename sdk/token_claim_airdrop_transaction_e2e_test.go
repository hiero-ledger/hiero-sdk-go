...
func TestIntegrationTokenClaimAirdropCanExecute(t *testing.T) {
	...
	waitForMirrorTokenBalance(t, env, receiver, tokenID, tokenClaimAirdropTransferAmount)
	waitForMirrorTokenBalance(t, env, receiver, nftID, 2)

	// Verify the operator does not hold the tokens
	waitForMirrorTokenBalance(t, env, env.OperatorID, tokenID, 1_000_000-tokenClaimAirdropTransferAmount)
	waitForMirrorTokenBalance(t, env, env.OperatorID, nftID, 8)
}
...
func TestIntegrationTokenClaimAirdropMultipleReceivers(t *testing.T) {
	...
	waitForMirrorTokenBalance(t, env, receiver1, tokenID, tokenClaimAirdropTransferAmount)
	waitForMirrorTokenBalance(t, env, receiver1, nftID, 2)

	// Verify the receiver2 holds the tokens via query
	waitForMirrorTokenBalance(t, env, receiver2, tokenID, tokenClaimAirdropTransferAmount)
	waitForMirrorTokenBalance(t, env, receiver2, nftID, 2)

	// Verify the operator does not hold the tokens
	waitForMirrorTokenBalance(t, env, env.OperatorID, tokenID, 1_000_000-tokenClaimAirdropTransferAmount*2)
	waitForMirrorTokenBalance(t, env, env.OperatorID, nftID, 6)
