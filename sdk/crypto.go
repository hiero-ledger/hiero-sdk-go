...
func (pk PublicKey) VerifySignedMessage(message []byte, signature []byte) bool {
	if pk.ecdsaPublicKey != nil {
		return pk.ecdsaPublicKey._VerifySignedMessage(message, signature)
	}
...
// Deprecated: Use [PublicKey.VerifySignedMessage] instead. Verify is not
// keytype agnostic and has different behavior for ed25519 and ecdsa
// keys, the latter requiring a [Keccak256Hash] bytes as the message.
