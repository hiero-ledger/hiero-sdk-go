...
func TestIntegrationContractUpdateTransactionCanRemoveAutoRenewAccount(t *testing.T) {
	t.Parallel()
	env := NewIntegrationTestEnv(t)
	defer CloseIntegrationTestEnv(env, nil)
	...
	resp, err = NewContractCreateTransaction().
		...
		SetGas(contractDeployGas).
		SetConstructorParameters(NewContractFunctionParameters().AddString("hello from hiero")).
		SetAutoRenewAccountID(env.Client.GetOperatorAccountID()).
		SetBytecodeFileID(fileID).
		SetContractMemo("[e2e::ContractCreateTransaction]").
		Execute(env.Client)
	require.NoError(t, err)
	...
	assert.NotNil(t, info)
	assert.Equal(t, *info.AutoRenewAccountID, env.Client.GetOperatorAccountID())
